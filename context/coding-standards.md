...
- Use spread operator for object and array copies
- Use `map()` for transformations, `filter()` for filtering, and `reduce()` for aggregation
- Use `find()` and `findIndex()` for searching
...
- Use `toLowerCase()` and `toUpperCase()` for case-insensitive comparisons
