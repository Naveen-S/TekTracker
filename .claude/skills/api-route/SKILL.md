---
name: api-route
description: Scaffold a new endpoint (Route Handler or Server Action) following the established auth-layer patterns — zod validate() at the boundary, requireUser()/isAdmin guards, the right response contract, force-dynamic, Jira specifics isolated. Use when adding any API route, server action, or domain mutation (migration step 4+).
---

# New endpoint — Route Handler or Server Action

## Choose the mechanism first (coding-standards + auth-layer decision 7)

- **Route Handler** (`src/app/api/<path>/route.js`): third-party integrations (Jira), webhooks,
...
- **Server Action** (`src/actions/<feature>.js`): in-app form submissions and simple mutations
...
## Route Handler pattern (exemplar: `src/app/api/auth/login/route.js`)

1. **Boundary validation**: zod schema in `src/lib/schemas/<feature>.js` (+ JSDoc `@typedef`
...
5. **Jira isolation (§17)**: anything Jira — base URL, Basic-auth construction, endpoints, custom
   field IDs (`customfield_10008`/`_10020`), pagination — lives in `src/lib/jira/client.js`
...
200) against `yarn dev` on :3002, then run /verify (build must stay DB-free and the route must
list as `ƒ (Dynamic)`).
